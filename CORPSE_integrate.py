...
def fsolve_wrapper(SOM_list,T,theta,Ndemand,Ctransfer,inputs,clay,params):
    ...
    deriv=CORPSE_deriv.CORPSE_deriv(SOM_dict,T,theta,Ndemand,Ctransfer,params,claymod=CORPSE_deriv.prot_clay(clay)/CORPSE_deriv.prot_clay(20))
...
def ode_wrapper(SOM_list,time,T,theta,Ndemand,Ctransfer,inputs,clay,params):
    return fsolve_wrapper(SOM_list,T,theta,Ndemand,Ctransfer,inputs,clay,params)

def arrayify_dict(d):
    from numpy import atleast_1d
    return dict(((v,atleast_1d(d[v])) for v in d))

def run_CORPSE_ODE(T,theta,Ndemand,Ctransfer,inputs,clay,initvals,params,times):
    ...
    result=odeint(ode_wrapper,ivals,times,
        args=(T+273.15,theta,Ndemand,Ctransfer,inputs,clay,params))
